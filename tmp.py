...
class Intervention():
    ...
    def __init__(self,
                 ...
                 substitutes: list, # [?¡À?¡À, 'man', 'woman']
        ...
        self.base_strings_tok = [
            # ?¡×???¡© base¢¬? ?¢´?? Template?? Encoding (?¢´?? index¢§? encoding)
            self.enc.encode(s,
...
class Model():
    ...
    def __init__(self,
        ...
        # Options
        self.top_k = 5
        self.num_layers = self.model.config.num_hidden_layers # model?? hidden layer¡Æ©©??¢¬? ?? ?? ??¢¥?. , gpt2,bert?? ¡Æ??? 12
        self.num_neurons = self.model.config.hidden_size # model?? hidden vector size¢¬? ?? ?? ??¢¥?. , gpt2,bert?? ¡Æ??? 768
        self.num_heads = self.model.config.num_attention_heads # model?? attention heads ¡Æ©©??¢¬? ?? ?? ??¢¥?. , gpt2,bert?? ¡Æ??? 12
    ...
    def get_representations(self, context, position):
        ...
        def extract_representation_hook(module,
            ...
            # ¡Æ¢¨ layer¢¬? ??¡Æ????¡×¢¬?¢¥? 0???¡Æ token¡Æ? {} token?? embedding vector¢¬? representation?? ????©©?¢¬?¢§? ????
        ...
        with torch.no_grad():
            ...
            # word embdedding?? forward ?????? ?©©¢¬??? forward_hook?? handle
            # ??¢¬? partial¢§? argument¢¬? ??¢¬? ?¢´?? ????¢§? ¢¬¢¬???? ©©??¢¬¢§?¢©? handle?? ??¢¬? ????
            # extract_representation_hook?? module = word_emb_layer
            handles.append(self.word_emb_layer.register_forward_hook(
                partial(extract_representation_hook,
                        position=position,
                        representations=representation,
                        layer=-1)))
            # hidden layers -> gpt2?? ¡Æ??? 12¡Æ©©
            for layer in range(self.num_layers):
                # ¡Æ¢¨ gpt2 ¢¬©¢?¡§?? hidden layer 12¡Æ©©?©¬ ??©©??? block?? module¢§? ????
                # ?¡×?? ¡Æ¡Æ?? handle¢§? control???? ?¡×???¡© partial¢§? argument¢¬? ??¢¬? ?¢´?? ????¢§? ¢¬¢¬???? ©©?
                # extract_representaion_hook?? modeul = ¡Æ¢¨ gpt2 layer block
            ...
            if self.is_xlnet:
                ...
                # context¢¥? {}¡Æ? ?¢´???? ?????? ?¢´??
                # ??¡Æ? forward?? ????¡Æ?¢¬??¡© ?¡×???¡© ?©©?¢´?? ©©??? forward_hook?? ????¡Æ¡§?¢¬¢§?¢©?
                # ¡Æ¢¨ layer¢¬?¢¥??? representation?? {}????©©?¢¬? ????¢§? ??????¡Æ? ??¢¥?.
    ...
    def get_probabilities_for_examples(self, context, candidates):
        ...
        outputs = [c[0] for c in candidates] #["he","she"]¡Æ? ?? ??¢§?
        if self.is_xlnet:
            logits = self.xlnet_forward(context, clen=1)[0]
        else:
            logits = self.model(context)[0] #logit?? ©©??¢¬©©?
        logits = logits[:, -1, :] # ¢¥??¢©?? ?? ¢¥????? ??¢§?¢§?¢©? ¢¥????????? ??¢§??? ????
        probs = F.softmax(logits, dim=-1) # ??¢§??©©¢¬?
        return probs[:, outputs].tolist() # he ??¢¥? she¡Æ? ?? ??¢§??? return
    ...
    def neuron_intervention(self,
                            context,# {}¢¬? ¢¥????? ???? token ????
                            outputs,# "he","she"¢¬? ?????? ?¢´???? ?????? list¢§? ??¢­?
                            rep,# ?????? ?????? ¡Æ¢¨ layer?¡Æ representation
                            layers,# search?? layer list e.g> [-1]
                            neurons,# search?? neuron list e.g> [[1,2,3]]
                            position,# {} ?¡×??
                            intervention_type='diff',# replace ???? diff ????
                            alpha=1.):
        ...
        def intervention_hook(module,
                              ...
                              position, # {} ?¡×??
                              neurons, # neuron?? ?¢´¢§? ¢¬?¢©???
                              intervention, # rep?? ??¢¥???¢¥? neuron?? alpha¢¬? ¡Æ???
                              intervention_type):
            # XLNet: ignore the query stream
            if self.is_xlnet and output.shape[0] == 1: return output
        ...
        for layer in set(layers):
            ...
            # layer?? ¡Æ¢¨ neuron?? ??¢¥???¢¥? ?????? alpha¢¬? ¡Æ???
            intervention_rep = alpha * rep[layer][n_list]
            if layer == -1:
                # forward hook?? ¡Æ?¢¥???, module?? word_emb_layer
                # ?? forward hook?? ¡Æ¢¨ layer?? ¢¥??? output?? ¡Æ??¢©?¢¬¢§?¢©? 
                ## replace?¢´?? diff ??¢¥? ??¡Æ?¢¬? ©©¢©¢¥?.
                handle_list.append(self.word_emb_layer.register_forward_hook(
                    partial(intervention_hook,
                            position=position, # {} ?¡×??
                            neurons=n_list, # neuron?? ?¢´¢§? ¢¬?¢©???
    ...
    def neuron_intervention_experiment(self,
                                       word2intervention, # {"?¡À?¡À1": Intervention¡Æ¢¥??1,"?¡À?¡À2":Intervention¡Æ¢¥??2,...}
    ...
    def neuron_intervention_single_experiment(self,
                                              intervention, # ?¡À?¡À?? ¢¥??? Intervention ¡Æ¢¥??
        ...
        with torch.no_grad():
            ...
            if self.is_bert or self.is_distilbert or self.is_roberta or self.is_xlnet:
                num_alts = intervention.base_strings_tok.shape[0] # ?¡À?¡À,man,woman?? Template?? Encoding¢©??¢­ ???¡©?? size(0) ?? 3
                # Special token id's(st_ids): (mask, cls, sep)
                ## mask ?????? ????
            ...
            # ¡Æ?¡Æ????¢¬¢§? get_representations???¡©¢¥? hook function?? ???????¡©
            ## ¡Æ¢¨ layer¢¬? ??¡Æ??????¡×?? base_string(profession) ?????? representation ????©©?¢¬?¢¬? ¢¬?????¢¥?¢¥?.
            ...
            # intervention.position == {}?¡×??
            # ¡Æ?¡Æ????¢¬¢§? get_representations???¡©¢¥? hook function?? ???????¡©
            ## ¡Æ¢¨ layer¢¬? ??¡Æ??????¡×?? base_string(man) ?????? representation ????©©?¢¬?¢¬? ¢¬?????¢¥?¢¥?.
            ...
            # intervention.position == {}?¡×??
            # ¡Æ?¡Æ????¢¬¢§? get_representations???¡©¢¥? hook function?? ???????¡©
            ## ¡Æ¢¨ layer¢¬? ??¡Æ??????¡×?? base_string(woman) ?????? representation ????©©?¢¬?¢¬? ¢¬?????¢¥?¢¥?.
            ...
            if intervention_loc == 'all':
              ...
              for layer in range(-1, self.num_layers):
                ...
                for neurons in batch(range(self.num_neurons), bsize):
                    ...
                    # diff or replace?? ?????? "he" or "she"¡Æ? ©©??? ??¢§??? ????
                    probs = self.neuron_intervention(
                        context=context, # {}¢¬? ¢¥????? ???? token ????
                        outputs=intervention.candidates_tok, # "he","she"¢¬? ?????? ?¢´???? ?????? list¢§? ??¢­?
                        rep=rep, # ?????? ?????? ¡Æ¢¨ layer?¡Æ representation
                        layers=layers_to_search, # search?? layer list e.g> [-1]
                        neurons=neurons_to_search, # search?? neuron list e.g> [[1,2,3]]
                        position=intervention.position, # {} ?¡×??
                        intervention_type=replace_or_diff, # replace ???? diff ????
                        alpha=alpha)
                    # ?????? ??¢§??? ????
